...
| **Extrapolation domain** | Narrower training domain for extrapolation evaluation | Time Dilation's constraint (v < c) leaves no room for extrapolation beyond v = 0.99c. Training domain is restricted to allow for safe out-of-distribution evaluation. |
...
|---|---|---|
| 2026-08-26 | Implemented narrower training domain for extrapolation evaluation (e.g. `EXTRAP_TRAIN_DOMAIN` vs `EXTRAP_TEST_DOMAIN`). | Extrapolation is evaluated on a narrower training domain than the recovery-rate experiment because Time Dilation's physical constraint (v < c) leaves no room for a safe extrapolation target beyond v = 0.99c. |
