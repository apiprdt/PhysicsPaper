...
==============================================================================
Pre-registered fair comparison harness: ADCD vs PySR (noise-robustness claim).

PRE-REGISTRATION NOTICE
--------------------------------------------------------------------------
All design decisions below were locked before any experimental results were
observed. Any post-hoc deviations must be recorded with explicit justification
in the companion document ADCD_vs_PySR_Workshop_Plan.md (Changelog section).
This discipline prevents unconscious p-hacking and is consistent with the
BIC/NMSE threshold calibration approach used throughout this project.

FAIRNESS PROTOCOL SUMMARY (see plan document for full rationale)
--------------------------------------------------------------------------
1. IDENTICAL DATA:
   generate_data() is called once with a shared seed and reused bit-for-bit
   by both methods. A determinism assertion verifies this at runtime.

2. SYMMETRIC RESIDUAL ISOLATION:
   Both methods search the correction residual (not the full law). The
   additive/multiplicative split is decided by detect_correction_mode() --
   the same internal function used by ADCD -- applied to the shared data.
   The scenario's correction_type field (ground truth) is NOT read directly.

3. EQUAL FEATURE ACCESS:
   PySR receives only scenario.classical_variables -- identical to ADCD.

4. DEFAULT PySR OPERATOR SET:
   Operators: +, -, *, /, exp, log, sqrt, sin, cos
   These are the out-of-the-box defaults representative of unconstrained SR
   usage. They are NOT tuned to match ADCD's physics-motivated primitive set.
   NOTE: PySR supports dimensional_constraint_penalty and TemplateExpression
   which could close part of the gap; evaluating those configurations is
   designated future work (see plan document, Scope section).

5. COMPUTE BUDGET:
   PySR receives timeout >= wall-clock time of the corresponding ADCD run
   (lower-bounded by MIN_PYSR_SECONDS). Any computational bias is in PySR's
   favour, not ADCD's.
   NOTE: deterministic=True and parallelism="serial" are required for
   reproducing identical PySR results across runs. This constraint reduces
   PySR candidate throughput relative to its parallel default. This is
   explicitly acknowledged in the paper's Methods section as a reproducibility
   trade-off; the timeout guarantee ensures PySR is not time-starved.

6. UNIFORM EVALUATION METRIC:
   classify_structure() from adcd.metrics is applied identically to both
   methods' best candidates. For PySR, numerical constants embedded in the
   expression are extracted and forwarded as theta_fit to ensure that the
   same classification logic is applied.

7. MULTI-SEED REPLICATION:
   Each (scenario, noise-level) point uses N_SEEDS independent seeds.
   Recovery rates are reported with 95% Wilson confidence intervals
   (more accurate than normal approximation near 0% or 100%).

8. EXTRAPOLATION METRIC (additional to recovery rate):
   Expressions found at the training domain boundary are evaluated on a
   held-out extrapolation domain (wider than training). This directly
   demonstrates asymptotic behaviour: ADCD's algebraically regularized
   primitives extrapolate correctly while polynomial approximations found
   by unconstrained GP diverge. This metric is not used for selection --
   it is a post-hoc diagnostic reported alongside recovery rate.

USAGE
--------------------------------------------------------------------------
    pip install pysr pandas matplotlib
    python eval/benchmark_pysr_comparison.py --engine julia
    python eval/benchmark_pysr_comparison.py --engine julia --include-ablation
    python eval/benchmark_pysr_comparison.py --scenario "Screened Coulomb"
    python eval/benchmark_pysr_comparison.py --plot-only run_outputs/pysr_comparison.json
==============================================================================
"""
...
# ==============================================================================
# Pre-registered configuration -- do not modify after first run without
# recording the deviation and justification in the plan document.
# ==============================================================================

NOISE_SWEEP: List[float] = [0.01, 0.02, 0.05, 0.10, 0.15, 0.20, 0.30]
DEFAULT_SEEDS: List[int] = [42, 43, 44, 45, 46]  # n=5 minimum for meaningful Wilson intervals

LOCKED_SCENARIOS: List[str] = ["Time Dilation", "Screened Coulomb", "Entropy Expansion"]

# Wide domains used for both training (broad signal range) and extrapolation.
# Extrapolation domain is 2x the training domain upper bound.
DEFAULT_CLEAN_DOMAINS: Dict[str, float] = {
    "Time Dilation": 0.99,     # training: v <= 0.3c; extrap: v in (0.3c, 0.9c]
    "Screened Coulomb": 4.0,
    "Entropy Expansion": 3.0,
}

# Extrapolation domain upper bounds (fraction of DEFAULT_CLEAN_DOMAINS value).
# Must be strictly greater than the training domain_max passed to each run.
EXTRAP_DOMAIN_MULTIPLIER: Dict[str, float] = {
    "Time Dilation": 0.90,      # evaluate on v in (0.3c, 0.90c]
    "Screened Coulomb": 8.0,    # evaluate on r in (4.0, 8.0] m
    "Entropy Expansion": 6.0,   # evaluate on dV/V_i in (3.0, 6.0]
}

# Out-of-the-box PySR operator set (not customised per scenario).
...
# ==============================================================================
# Statistics -- Wilson confidence interval for binary proportions
# ==============================================================================
...
def wilson_interval(
    ...
    More accurate than the normal approximation when n is small or when
    the true proportion is near 0 or 1 -- both of which occur at the
    extremes of the noise sweep (0% and 100% recovery).
...
# ==============================================================================
# Shared data layer -- single source of truth for ADCD and PySR
# ==============================================================================
...
class SharedData:
    ...
    target: np.ndarray          # residual in the space appropriate for detected_mode
    detected_mode: str          # "additive" | "multiplicative"
    feature_names: List[str]
    # Extrapolation hold-out (generated from wider domain, NOT seen during search)
...
def _assert_generate_data_deterministic(scenario, noise: float, seed: int, domain_max: float) -> None:
    """
    Runtime guard: verify that generate_data() produces byte-identical output
    for the same (noise, seed, domain_max) arguments. This directly validates
    the pre-registration claim that both methods receive identical data.
    Raises AssertionError on failure, which would indicate a non-deterministic
    data generator and must be fixed before any experimental results are trusted.
    """
    ...
    assert np.array_equal(y1, y2), (
        f"FATAL: generate_data() is not deterministic for scenario "
        f"'{scenario.name}' (seed={seed}, noise={noise}). "
        "Experimental results cannot be trusted until this is fixed."
    )
    assert np.array_equal(yc1, yc2), (
        "FATAL: generate_data() produced non-deterministic y_classical."
    )
...
def build_shared_data(
    ...
    """
    Build the shared data object used by both ADCD and PySR.

    Training data (noise, domain_max) and extrapolation data (no noise,
    extrap_domain_max) are generated here with a single call each, then
    stored in SharedData for reuse. The determinism assertion is executed
    once per (scenario, noise, seed) combination.
    """
    ...
    # Extrapolation hold-out: generated at wider domain, zero noise.
    X_extrap, target_extrap = None, None
    if extrap_domain_max is not None and extrap_domain_max > domain_max:
        # Seed offset (+1000) ensures the extrapolation sample is independent
        # from the training sample while remaining fully reproducible.
...
def _evaluate_expression_on_data(
    ...
    """
    Evaluate a SymPy expression string on a data dictionary.
    Returns None if evaluation fails (parse error, NaN/Inf, etc.).
    """
    try:
...
def _compute_nmse_extrap(
    ...
    """
    Extrapolation NMSE: evaluate the discovered expression on the held-out
    extrapolation domain (wider than training, zero noise).

    Returns inf if evaluation fails or extrapolation data is unavailable.
    This value is reported as a diagnostic -- it is NOT used for selection
    or for determining the recovery rate.
    """
...
def _extract_numeric_constants_as_theta_fit(expr: sp.Expr) -> Dict[str, float]:
    """
    Extract numeric constants embedded directly in a PySR expression
    and map them to dummy theta_N names, for use with classify_structure().

    PySR fits parameters inline as numeric literals (e.g. 1.492 * exp(-r/1.507)).
    classify_structure() uses theta_fit to handle degenerate exponents; without
    it, power-law expressions with fitted integer-looking exponents can be
    misclassified. This function provides a best-effort mapping.
    """
...
# ==============================================================================
# ADCD search path -- thin wrapper around the audited run_scenario_protocol
# ==============================================================================
...
def run_adcd_once(
    ...
    """
    Execute one ADCD run via run_scenario_protocol().

    A deep copy of the scenario is used to prevent engine attribute mutation
    from leaking into subsequent runs when scenarios are reused across the sweep.
    """
...
def _count_free_params_in_pysr_expr(
    ...
    """
    Count the number of free parameters (fitted numeric constants) in a PySR
    expression. PySR embeds fitted constants as numeric literals; feature
    variables and mathematical constants (pi, E) are not parameters.

    This is the correct value for n_params in BIC computation, NOT the
    PySR 'complexity' column, which counts AST nodes.
    """
    try:
        expr = sp.sympify(expr_str)
        feature_syms = {sp.Symbol(n) for n in feature_names}
        # Symbols that are not feature variables and not well-known constants
        known_consts = {sp.pi, sp.E, sp.I}
        free_syms = expr.free_symbols - feature_syms - known_consts
        # Count only genuine parameter symbols (theta_N) or unrecognised symbols
        # that PySR would have optimised. In PySR expressions these usually appear
        # as numeric literals, so symbol count is typically 0 here. The main
        # parameter count comes from the numeric constant count below.
        n_symbol_params = len(free_syms)

        # Count distinct non-integer numeric literals as fitted parameters.
        numeric_consts = set()
        for node in sp.preorder_traversal(expr):
            if isinstance(node, sp.Number) and not node.is_Integer:
...
def run_pysr_once(
    ...
    """
    Execute one PySR run on the shared residual target.

    Configured with default out-of-the-box operators (see module header).
    deterministic=True and parallelism="serial" are required for
    cross-seed reproducibility; this reduces candidate throughput relative
    to PySR's parallel default (acknowledged in paper Methods section).
    """
    ...
    with warnings.catch_warnings():
        ...
        model = PySRRegressor(
            ...
            model_selection="best",     # PySR's internal criterion -- not overridden
    ...
    # Extract fitted numeric constants to improve classify_structure accuracy.
    ...
    return {
        ...
        # Retained internally for Phase 2 ablation reselection; removed before JSON serialisation.
...
def run_pysr_bic_reselect(
    ...
    """
    Phase 2 ablation (partial): reselect the best candidate from PySR's own
    Pareto front using ADCD's extended BIC (with multiple-testing penalty
    2*ln(n_candidates)), instead of PySR's internal selection criterion.

    This isolates the contribution of ADCD's decision framework (extended BIC
    + identifiability gating) from its physics grammar. If PySR+ADCD_BIC
    falls strictly between PySR and full ADCD in recovery rate, it suggests
    the statistical framework itself (not only the grammar) contributes to
    ADCD's advantage.

    Note: n_params is computed from the SymPy expression's free parameter
    count, NOT from PySR's 'complexity' column (which measures AST node count,
    ...
    for _, row in eqs.iterrows():
        try:
            # PySR loss column stores MSE by default.
            loss = float(row["loss"])
            nmse = loss / var_y

            # Correct parameter count: free parameters, not AST complexity.
...
def run_one_combination(
    ...
    """
    Execute one (scenario, noise, seed) combination for both ADCD and PySR.

    PySR timeout is set to max(adcd_elapsed, min_pysr_seconds) to guarantee
    PySR is never time-starved relative to ADCD.
    """
    ...
    # Build shared data once; both methods read from this object.
    ...
    # Compute extrapolation NMSE for both methods.
    ...
    # Remove non-serialisable internal fields before JSON dump.
...
def aggregate_summary(
    ...
    """
    Aggregate raw run results into recovery rates with Wilson 95% CIs,
    grouped by (scenario, noise_level).
    """
    ...
    for sc_name in targets:
        ...
        for noise in noise_sweep:
            ...
            # Median extrapolation NMSE across seeds (robust to outliers).
...
# ==============================================================================
# Plotting -- 3-panel headline figure + optional extrapolation figure
# ==============================================================================
...
def plot_results(
    ...
    """
    Generate the headline 3-panel recovery-vs-noise figure (PDF + PNG) and
    an optional extrapolation NMSE comparison figure.

    Output files:
        <out_dir>/fig_comparison_recovery.pdf / .png
        <out_dir>/fig_comparison_extrap.pdf / .png (if extrapolation data available)
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    # Extrapolation NMSE figure (if data available).
    if has_extrap:
...
def print_summary_table(
    ...
    """Print a formatted recovery-rate table to stdout."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Pre-registered ADCD vs PySR noise-robustness comparison harness.\n"
            "See ADCD_vs_PySR_Workshop_Plan.md for protocol rationale."
        ),
    ...
    parser.add_argument(
        ...
        help="Scenario name to run, or 'all' for all three locked scenarios.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Minimum PySR timeout (seconds) regardless of ADCD runtime.",
    )
    parser.add_argument(
        "--include-ablation", action="store_true",
        help="Run Phase 2 PySR+ADCD_BIC reselection experiment.",
    )
    parser.add_argument(
        "--no-extrap", action="store_true",
        help="Disable extrapolation NMSE computation (faster, less informative).",
    )
    parser.add_argument(
        "--out", default=None,
    ...
    parser.add_argument(
        ...
        help="Skip experiments; load existing JSON and regenerate figures only.",
    )
    ...
    # Plot-only mode: load existing results and regenerate figures.
    if args.plot_only:
    ...
    print("=" * 92)
    print(
        f" ADCD vs PySR -- PRE-REGISTERED NOISE ROBUSTNESS COMPARISON\n"
        f" Total runs: {total}  "
        f"({len(targets)} scenario(s) x {len(args.noise_sweep)} noise levels x {len(args.seeds)} seeds)\n"
        f" PySR operator set: {PYSR_BINARY_OPERATORS + PYSR_UNARY_OPERATORS}\n"
        f" Extrapolation NMSE: {'disabled (--no-extrap)' if args.no_extrap else 'enabled'}"
    )
    print("=" * 92)
