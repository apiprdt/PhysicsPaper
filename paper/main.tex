...
by construction.
Candidates are assembled by deterministic grammar enumeration and filtered
through a four-gate physical and epistemic cascade before any numerical
fitting.
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Gate D (Fine parameter fit):} Surviving candidates undergo full continuous parameter optimization using \texttt{Optim.jl} Nelder--Mead ($n_\mathrm{restarts} = 15$) in \texttt{float64} precision with symmetric log-uniform parameter initializations to avoid local minima.
...
Surviving candidates are ranked by the Extended Bayesian Information Criterion (EBIC)~\cite{chen2008extended}, which incorporates a complexity penalty for the size of the candidate dictionary $|\mathcal{C}|$:
\begin{equation}
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Gate V.2 (Positive control):} The Rank-1 candidate must recover the correct asymptotic power-law or limiting slope on an asymptotic sub-domain ($u \to 0$), verifying that numerical optimization did not settle into an unphysical local minimum.
  \item \textbf{Gate V.3 (Ablation control):} An ablated search is performed with the winning primitive class removed from the dictionary. The true model must outperform the best available alternative by a decisive Bayesian margin ($\Delta\mathrm{BIC}_\mathrm{ablation} = \mathrm{BIC}_\mathrm{ablated} - \mathrm{EBIC}_\mathrm{candidate} \ge 10.0$).
  \item \textbf{Gate V.4 (Determinism check):} Three independent runs with identical inputs produce byte-exact identical numerical and symbolic results via deterministic cryptographic seeding of the optimizer's \texttt{MersenneTwister}.
\end{enumerate}
...
\begin{itemize}[noitemsep,leftmargin=*]
  ...
  \item \textbf{\texttt{WITHHELD}:} The data are consistent with pure Gaussian noise around the classical baseline ($\Delta\mathrm{BIC}_\mathrm{gate} < 10.0$), non-convergence, or determinism failure, confirming that the classical baseline alone is sufficient.
\end{itemize}
...
\Function{EpistemicDiscovery}{$y_\mathrm{obs}, y_\mathrm{cl}, \mathbf{X}, \mathcal{U}, \Omega, \Delta\mathrm{BIC}^*$}
    ...
    \For{\textbf{each} candidate $c \in \mathcal{C}$}
        \If{$\textsc{IsDimensionlessRelaxed}(c, \mathcal{U})$ \textbf{and} $\textsc{AsymptoticProbe}(c, 10^{-12}) < 10^{-4}$}
            ...
            \If{$\mathrm{NMSE}(\boldsymbol{\theta}_c^\mathrm{coarse}) \le 1.0$}
                \State $\boldsymbol{\theta}_c^* \leftarrow \textsc{NelderMead}(c, \Delta, \text{restarts}=15)$ \Comment{fine fit}
    ...
    \EndIf
    \State $\hat{c}, \boldsymbol{\theta}^*, \mathrm{EBIC}^* \leftarrow \arg\min_{(c,\boldsymbol{\theta},\mathrm{EBIC}) \in \mathcal{C}_\mathrm{valid}} \mathrm{EBIC}$
    \State $\Delta\mathrm{BIC}_\mathrm{gate} \leftarrow \mathrm{BIC}_\mathrm{null} - \mathrm{EBIC}^*$
    \If{$\Delta\mathrm{BIC}_\mathrm{gate} < \Delta\mathrm{BIC}^*$}
    ...
    \State $\text{pass}_\mathrm{det} \leftarrow \textsc{DeterminismCheck}(\hat{c})$
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Noise-level sweep} ($7$ levels from $1\%$ to $30\%$, $n = 5$ seeds, $105$ runs total): characterizes how observation noise modulates structural recovery.
\end{enumerate}
...
We evaluate out-of-domain generalization to examine whether asymptotic
regularization (a design feature of ADCD but not of general
SR) translates to improved extrapolation consistency.
Held-out extrapolation evaluation is conducted strictly on non-overlapping
extended domains: $v \in (0.3c, 0.8c]$ for Time Dilation, $r \in (4.0, 8.0]$
for Screened Coulomb, and $dV/V_i \in (3.0, 6.0]$ for Entropy Expansion.
ADCD's expressions, constrained to vanish at the classical limit by
construction, maintain physical consistency outside the training domain:
across the zero-noise experiment ($n = 20$ seeds), median extrapolation
NMSE is below $10^{-7}$ for all three scenarios.
PySR's expressions, not designed with extrapolation constraints, exhibit
variable behavior depending on the functional form selected, which is a direct
and expected consequence of the two frameworks' different design
priorities.
The most pronounced illustration is in the Screened Coulomb zero-noise
experiment: PySR's worst-case extrapolation NMSE reaches $1530.5$ across
$20$ seeds (median: $0.054$), reflecting runs where the selected
expression diverges outside the training domain.
...
\begin{table*}[t]
  ...
  an identifiability claim when accompanied by passing positive and determinism controls.
