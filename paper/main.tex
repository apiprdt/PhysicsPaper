...
\textbf{Output:} Epistemic verdict $v \in \{\texttt{IDENTIFIABLE}, \texttt{DETECTED\_\allowbreak UNRESOLVED}, \texttt{WITHHELD}\}$, certified expression $\hat{\Delta}$, parameter estimates $\boldsymbol{\theta}^*$, Bayes factors, machine-readable audit trail.
\vspace{3pt}
\hrule height 0.4pt
\vspace{4pt}
\begin{algorithmic}[1]
\Function{EpistemicDiscovery}{$y_\mathrm{obs}, y_\mathrm{cl}, \mathbf{X}, \mathcal{U}, \Omega, \Delta\mathrm{BIC}^*$}
    ...
    \Else
        \State \Return $\langle\texttt{DETECTED\_\allowbreak UNRESOLVED},\; \hat{c},\; \boldsymbol{\theta}^*,\; \Delta\mathrm{BIC}_\mathrm{ablation}\rangle$
    \EndIf
\EndFunction
\end{algorithmic}
\vspace{3pt}
...
\texttt{DETECTED\_\allowbreak UNRESOLVED} verdict if $\Delta\mathrm{BIC}_\mathrm{ablation} < 10$; in that case, the
...
\begin{table*}[t]
  ...
  \begin{ruledtabular}
    ...
    Time Dilation ($v \le 0.3c$)
      ...
      & $-160.88$
      & $-0.74$
      & \texttt{DETECTED\_\allowbreak UNRESOLVED}\footnote{Failed Gate V.2 (positive control) and Gate V.3 (ablation margin $\Delta\mathrm{BIC} = -0.74 < 10.0$).} \\[6pt]
    Screened Coulomb ($r \le 4.0$)
      & $\theta_2\,(1 - e^{-r/\theta_0})$
      & $-1617.66$
      & $25.74$
      & \texttt{DETECTED\_\allowbreak UNRESOLVED}\footnote{Passed Gate V.3 ($\Delta\mathrm{BIC} = 25.74$), but failed Gate V.2 (positive control at $1\%$ noise level).} \\[4pt]
...
ADCD assigns a verdict of \texttt{DETECTED\_\allowbreak UNRESOLVED}, providing transparent diagnostic feedback that anomalous physics exists but cannot be formally certified without broader domain data.
For Screened Coulomb, the recovered Debye length $\theta_0 \approx 1.50$
matches the ground truth $\lambda_D = 1.50$ and achieves decisive ablation margin ($\Delta\mathrm{BIC} = 25.74$), though positive control sensitivity at $1\%$ noise yields a \texttt{DETECTED\_\allowbreak UNRESOLVED} tier in blind mode.
...
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
Regime & \textsc{Identifiable} & \shortstack{\textsc{Detected}\\\textsc{Unresolved}} & \textsc{Withheld} \\
\hline
...
ADCD assigns a qualified verdict of \texttt{DETECTED\_\allowbreak UNRESOLVED} rather than reporting an unsupported
discovery.
...
The \texttt{WITHHELD} tier is reserved for cases where the classical baseline itself is sufficient (consistent with the $\Delta = 0$ null model).
