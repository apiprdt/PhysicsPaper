...
\newcommand{\candidate}{\texttt{CANDIDATE}}
\newcommand{\identifiable}{\texttt{IDENTIFIABLE}}
\newcommand{\withheld}{\texttt{WITHHELD}}
...
\begin{abstract}
    ...
    General unconstrained symbolic regression engines (such as PySR) are designed to recover complete
    ...
    and noisy conditions ($1\%$--$30\%$ Gaussian noise). Crucially, ADCD explicitly differentiates between structural search success and formal epistemic certification: across all 165 runs, ADCD formally certified 112 runs as \identifiable, explicitly qualified 53 runs as \candidate due to SNR-limited ambiguity, and produced zero false-positive structural certifications across all noise-degraded and unresolved regimes.
\end{abstract}
...
\begin{table*}[t]
\caption{Conceptual comparison of scientific discovery paradigms. ADCD restricts its scope to corrections of known physical baselines and enforces boundary limits by construction.}
\label{tab:paradigms}
\begin{ruledtabular}
\small
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llll}
\textbf{Method} & \textbf{Search Space} & \textbf{Boundary Constraint} & \textbf{Validation / Model Selection} \\
\hline
PySR~\cite{cranmer2023}         & Free expression tree        & None                             & Complexity-penalized Pareto fit \\
SINDy~\cite{brunton2016}        & Polynomial library          & None                             & Sequential thresholded regression \\
PINNs~\cite{raissi2019}         & Neural network              & Soft PDE residual loss           & Multi-objective gradient loss \\
AI Feynman~\cite{udrescu2020}   & Free tree + symmetry        & Dimensional/separability tests   & Pareto frontier selection \\
AI Descartes~\cite{cornelio2023}& Axiom theory + data         & Logical physical axioms          & Formal logical derivability \\
\textbf{ADCD (ours)}            & Regularized primitive dict. & \textbf{Exact algebraic ($\lim_{u\to 0} D=0$)} & \textbf{Formal 4-step epistemic audit} \\
\end{tabular*}
\end{ruledtabular}
\end{table*}
...
\caption{The ADCD discovery and certification pipeline. Raw inputs $(y_\mathrm{obs}, y_\mathrm{cl}, \mathbf{X}, \mathcal{U})$ enter Stage~1 (dynamic mode selection via Spearman $|\rho|$). Stage~2 generates dimensionless arguments via Buckingham-$\pi$ with degree pruning and applies 6 structural templates. Stage~3 screens candidates through a tiered filter cascade (dimensional relaxation, numerical asymptotic boundary probe, coarse/fine Nelder--Mead parameter optimization). Stage~4 ranks surviving candidates by EBIC and applies the Gate~E null-model test ($\Delta\mathrm{BIC}_\mathrm{gate} \ge 10.0$). Stage~5 applies the 4-step post-fitting epistemic validation protocol (budget disclosure, positive control, ablation margin $\Delta\mathrm{BIC}_\mathrm{ablation} \ge 10.0$, and computational reproducibility), yielding a 3-tier verdict (\identifiable, \candidate, or \withheld).}
\label{fig:pipeline}
\end{figure*}
...
($11\%$ relative error at $u = 2 \times 10^{-15}$; total collapse to $0.0$ at $u \le 10^{-17}$).
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Gate D (Fine parameter fit):} Surviving candidates undergo full continuous parameter optimization using native \texttt{Optim.jl} Nelder--Mead ($n_\mathrm{restarts} = 50$) in the core Julia engine (a pure-Python/JAX backend is also included in the repository for rapid prototyping and gradient cross-validation) in \texttt{float64} precision across five initialization scales ($10^{-6}$ to $10^6$) with quadrant sign permutations and log-uniform exponent sampling to robustly escape non-convex local minima.
...
The candidate is certified at Gate~E if it outperforms the classical null baseline by $\Delta\mathrm{BIC}_\mathrm{gate} = \mathrm{BIC}_\mathrm{null} - \mathrm{EBIC}_\mathrm{corr} \ge 10.0$ and satisfies the noise-adaptive fit-quality ceiling:
\begin{align}
  \mathrm{NMSE}^* &\le \mathrm{NMSE}_\mathrm{max} \notag\\
  &= \operatorname{clip}\!\left(1.35\,\sigma_\mathrm{n}^2 + 1.645\,\sigma_\mathrm{n}^2 \sqrt{2/N_\mathrm{eff}},\, 0.10,\, 0.60\right)
\end{align}
where $\sigma_\mathrm{n}$ denotes the measurement noise level, $1.35\,\sigma_\mathrm{n}^2$ accounts for the expected residual sum-of-squares under Gaussian perturbations with a 35\% inflation margin for finite-sample non-normality, $1.645\,\sigma_\mathrm{n}^2 \sqrt{2/N_\mathrm{eff}}$ represents the one-sided 95\% upper confidence bound ($z_{0.95} = 1.645$) on the sampling variance of the $\chi^2$ distribution, and clipping to $[0.10, 0.60]$ enforces an absolute sanity floor and ceiling preventing runaway divergent fits.

ADCD additionally implements Bayesian Model Averaging (BMA) via \texttt{bayesian\_ranker.py} --- computing posterior model weights, structural class probabilities, and posterior entropy to quantify ambiguity among competing functional forms. BMA outputs are available in the full audit trail but are not the primary quantity reported in the benchmarks of this paper, which focus on structural recovery rate and EBIC-based model selection.
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Gate V.2 (Positive control):} The optimizer is evaluated on a restricted candidate space containing only the isolated target primitive family, verifying that numerical optimization reliably converges to low loss ($\mathrm{NMSE} \le 0.1$) when given the discovered primitive family of its top-ranked candidate alone (operating autonomously without ground-truth knowledge).
  ...
  \item \textbf{Gate V.4 (Cross-seed structural stability):} Verifies that the top-ranked functional class is robustly recovered across three independent synthetic noise realizations (seeds $s$, $s+1000$, and $s+2000$) with fitted parameter variation $\max_i |\theta_i^{(s)} - \theta_i^{(s')}| < 10^{-4}$, certifying that the discovered functional form is invariant to specific random noise draws.
\end{enumerate}

ADCD classifies discovery results into three formal epistemic outcome tiers:
\begin{itemize}[noitemsep,leftmargin=*]
  \item \textbf{\identifiable:} The candidate passes all four validation gates with decisive evidence ($\Delta\mathrm{BIC}_\mathrm{ablation} \ge 10.0$) and verified positive control.
  \item \textbf{\candidate:} Strong evidence of a physical anomaly is confirmed against the classical null model ($\Delta\mathrm{BIC}_\mathrm{gate} \ge 10.0$), but the candidate fails either Gate V.2 (Positive control) or Gate V.3 (Ablation control margin $< 10.0$). This outcome explicitly informs the researcher that real physics beyond the baseline exists, but the observation domain or SNR is insufficient to unequivocally isolate the true functional form from competing alternatives.
  \item \textbf{\withheld:} The data are consistent with pure Gaussian noise around the classical baseline ($\Delta\mathrm{BIC}_\mathrm{gate} < 10.0$), non-convergence, or reproducibility failure, confirming that the classical baseline alone is sufficient.
\end{itemize}
...
\hrule height 0.4pt
\vspace{4pt}
\noindent\textbf{Input:} Observed array $y_\mathrm{obs}$, classical baseline $y_\mathrm{cl}$, feature matrix $\mathbf{X} = \{x_j\}$, SI unit declarations $\mathcal{U}$, observation window $\Omega$, evidence threshold $\Delta\mathrm{BIC}^* = 10.0$, calibrated fit-quality ceiling $\mathrm{NMSE}_\mathrm{max}$ (noise-adaptive; see Section~\ref{sec:cascade}).\\
\textbf{Output:} Epistemic verdict $v \in \{\identifiable, \candidate, \withheld\}$, certified expression $\hat{\Delta}$, parameter estimates $\boldsymbol{\theta}^*$, Bayes factors, machine-readable audit trail.
\vspace{3pt}
\hrule height 0.4pt
\vspace{4pt}
\begin{algorithmic}[1]
\Function{EpistemicDiscovery}{$y_\mathrm{obs}, y_\mathrm{cl}, \mathbf{X}, \mathcal{U}, \Omega, \Delta\mathrm{BIC}^*$}
    ...
    \If{$\mathcal{C}_\mathrm{valid} = \emptyset$}
        \State \Return $\langle\withheld,\; \text{None},\; \text{None},\; 0.0\rangle$
    \EndIf
    ...
    \If{$\Delta\mathrm{BIC}_\mathrm{gate} < \Delta\mathrm{BIC}^*$ \textbf{or} $\mathrm{NMSE}^* > \mathrm{NMSE}_\mathrm{max}$}
        \State \Return $\langle\withheld,\; \hat{c},\; \boldsymbol{\theta}^*,\; \Delta\mathrm{BIC}_\mathrm{gate}\rangle$
    \EndIf
    ...
    \If{$\text{pass}_\mathrm{det}$ \textbf{and} $\text{pass}_\mathrm{pos}$ \textbf{and} $\Delta\mathrm{BIC}_\mathrm{ablation} \ge \Delta\mathrm{BIC}^*$}
        \State \Return $\langle\identifiable,\; \hat{c},\; \boldsymbol{\theta}^*,\; \Delta\mathrm{BIC}_\mathrm{ablation}\rangle$
    \Else
        \State \Return $\langle\candidate,\; \hat{c},\; \boldsymbol{\theta}^*,\; \Delta\mathrm{BIC}_\mathrm{ablation}\rangle$
    \EndIf
\EndFunction
\end{algorithmic}
\vspace{3pt}
...
\candidate verdict if $\Delta\mathrm{BIC}_\mathrm{ablation} < 10$; in that case, the
correct structure was found but the observation window or SNR is insufficient to unequivocally
certify the claim against competing functional alternatives.
...
Scenario & ADCD & PySR (default) & CI overlap \\
\hline
...
\begin{table*}[t]
  \caption{Single-seed epistemic validation diagnostic (seed~42, $1\%$ Gaussian noise, authoritative Julia engine in genuine blind search).
  All three scenarios recover the exact generating functional structure at Rank~1.
  ...
  Subscripts on $\theta_k$ reflect AST parameter assignment order during grammar enumeration;
  displayed expressions are algebraically simplified from raw AST templates by dropping terms whose fitted coefficient satisfies $|\theta_i| < 10^{-5}$ for presentation clarity.}
  \label{tab:results}
  \begin{ruledtabular}
    \small
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llrrc}
    \textbf{Scenario}
    ...
    Time Dilation ($v \le 0.3c$)
      & $\theta_1 \dfrac{\theta_0 (v^2/c^2)}{\sqrt{1 - \theta_0 v^2/c^2}\,(1 + \sqrt{1 - \theta_0 v^2/c^2})}$
      & $-386.95$
      & $+\infty$
      & \candidate\footnote{Held at \candidate tier by Gate V.2 (positive control SNR floor under $1\%$ noise on restricted historical domain $v \le 0.3c$, where anomalous signal $\le 4.5\%$).} \\[6pt]
    Screened Coulomb ($r \le 4.0$)
      & $\theta_1\,(1 - e^{-|r/\theta_0|})$
      & $-2073.49$
      & $+756.80$
      & \identifiable\footnote{Passed all four validation gates with decisive evidence $\Delta\mathrm{BIC} = +756.80 \ge 10.0$ and Debye length $\theta_0 \approx 1.50$ matching ground truth $\lambda_D = 1.50$.} \\[4pt]
    Entropy Expansion ($dV/V_i \le 3.0$)
      & $\theta_1\,\ln\!\left(1 + |\theta_0\tfrac{dV}{V_i}|\right)$
      & $-1406.76$
      & $+14.24$
      & \identifiable\footnote{Passed all four validation gates with decisive evidence $\Delta\mathrm{BIC} = +14.24 \ge 10.0$.} \\
    \end{tabular*}
  \end{ruledtabular}
\end{table*}

For Time Dilation at $v \le 0.3c$, the exact regularized Lorentz form is structurally recovered at
Rank~1 ($\mathrm{EBIC} = -386.95$). However, because the relativistic correction on this restricted historical domain
is $\le 4.5\%$ of the classical baseline while the noise floor is $1\%$, positive control explicitly flags that the anomaly is close to the local SNR floor.
ADCD assigns a qualified verdict of \candidate, providing transparent diagnostic feedback that an anomalous physical signal exists and is structurally captured at Rank~1, but warrants broader domain confirmation before formal unconstrained publication.
For Screened Coulomb, the recovered Debye length $\theta_0 \approx 1.50$
matches the ground truth $\lambda_D = 1.50$ with $\mathrm{EBIC} = -2073.49$ and decisive ablation margin ($\Delta\mathrm{BIC} = +756.80 \gg 10.0$), passing all four validation gates to achieve full \identifiable certification.
For Entropy Expansion, the logarithmic expansion achieves $\mathrm{EBIC} = -1406.76$ and passes all four gates with
$\Delta\mathrm{BIC} = +14.24 \ge 10.0$, achieving full \identifiable certification.
...
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
Regime & \textsc{Identifiable} & \textsc{Candidate} & \textsc{Withheld} \\
\hline
...
\subsection{Qualified detection and epistemic tiering as a design feature}

The Time Dilation scenario illustrates the
intended epistemic protocol: when the observational domain exhibits low signal-to-noise ratio
relative to the anomaly magnitude,
ADCD assigns a qualified verdict of \candidate rather than reporting an unsupported
or over-confident discovery.
This transparently informs the researcher that while an anomalous signal is detected relative to the baseline, the observation domain is insufficient to distinguish the relativistic Lorentz form from simpler polynomial approximations.
The \withheld tier is reserved for cases where the classical baseline itself is sufficient (consistent with the $\Delta = 0$ null model).
This tri-tier discrimination represents a deliberate design feature rather than a limitation, preventing false-positive scientific claims.
...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Extended scenario suite and observational connectors:} In addition to the three canonical benchmark scenarios evaluated in this paper, the open-source repository implements an extended suite of 21 multi-variable coupled scenarios (including Yukawa forces, nonlinear damping, and blind held-out identity batteries) as well as preliminary observational connectors for historical experimental datasets (Mercury perihelion precession, SPARC galactic rotation curves, and Pantheon+ supernovae). While systematic multi-seed benchmarking on this extended suite represents ongoing work, the open-source infrastructure is released to facilitate community expansion.
\end{enumerate}
...
ADCD was originally prototyped as a pure-Python/JAX implementation; the search and continuous optimization core has since been migrated to a native Julia (\texttt{Optim.jl}) production engine for high-throughput AST evaluation and robust parameter optimization (Section~\ref{sec:cascade}). The legacy Python/JAX path remains in the repository (selectable via \texttt{--engine python}) for cross-validation during migration but was not used to generate any of the results reported in this paper; all reported numbers use the authoritative Julia backend (\texttt{--engine julia}, the default). Where configuration defaults between the two backends differ (e.g., $n_\mathrm{restarts}$, $\mathrm{EBIC}$ thresholds), the Julia values documented in Section~\ref{sec:cascade} are authoritative.
...
ADCD returns an \texttt{ADCDResult} dataclass containing the epistemic verdict (\texttt{tier}: \texttt{"IDENTIFIABLE"}, \texttt{"CANDIDATE"}, or \texttt{"WITHHELD"}), \texttt{top\_expression} (SymPy string), \texttt{top\_latex}, \texttt{discovered\_class}, \texttt{nmse\_train}, \texttt{bic\_score}, \texttt{pareto\_front} (all surviving candidates ranked by BIC), and \texttt{checks} (complete machine-readable audit trail). \withheld does not mean ``no output''; the full Pareto front and diagnostic summary are always accessible.
...
print(result.tier)           # e.g., IDENTIFIABLE
