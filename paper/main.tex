...
\newcommand{\unresolved}{\texttt{DETECTED\_\allowbreak UNRESOLVED}}
...
\begin{abstract}
    ...
    and noisy conditions ($1\%$--$30\%$ Gaussian noise). Crucially, ADCD explicitly differentiates between structural search success and formal epistemic certification: across all 165 runs, ADCD formally certified 112 runs as \identifiable, explicitly qualified 53 runs as \unresolved due to SNR-limited ambiguity, and produced zero false-positive structural certifications across all noise-degraded and unresolved regimes.
\end{abstract}
...
\textbf{Output:} Epistemic verdict $v \in \{\identifiable, \unresolved, \withheld\}$, certified expression $\hat{\Delta}$, parameter estimates $\boldsymbol{\theta}^*$, Bayes factors, machine-readable audit trail.
\vspace{3pt}
\hrule height 0.4pt
\vspace{4pt}
\begin{algorithmic}[1]
\Function{EpistemicDiscovery}{$y_\mathrm{obs}, y_\mathrm{cl}, \mathbf{X}, \mathcal{U}, \Omega, \Delta\mathrm{BIC}^*$}
    ...
    \Else
        \State \Return $\langle\unresolved,\; \hat{c},\; \boldsymbol{\theta}^*,\; \Delta\mathrm{BIC}_\mathrm{ablation}\rangle$
    \EndIf
\EndFunction
\end{algorithmic}
\vspace{3pt}
...
\unresolved verdict if $\Delta\mathrm{BIC}_\mathrm{ablation} < 10$; in that case, the
...
\begin{table*}[t]
  ...
  \begin{ruledtabular}
    ...
    Time Dilation ($v \le 0.3c$)
      ...
      & $-160.88$
      & $-0.74$
      & \unresolved\footnote{Failed Gate V.2 (positive control) and Gate V.3 (ablation margin $\Delta\mathrm{BIC} = -0.74 < 10.0$).} \\[6pt]
    Screened Coulomb ($r \le 4.0$)
      & $\theta_2\,(1 - e^{-r/\theta_0})$
      & $-1617.66$
      & $25.74$
      & \unresolved\footnote{Passed Gate V.3 ($\Delta\mathrm{BIC} = 25.74$), but failed Gate V.2 (positive control at $1\%$ noise level).} \\[4pt]
    Entropy Expansion ($dV/V_i \le 3.0$)
      & $\theta_3\,\ln\!\left(1 + \tfrac{dV}{V_i}\right)$
      & $-811.34$
      & $37.99$
      & \identifiable\footnote{Passed all four validation gates (Gate V.1--V.4).} \\
    \end{tabular*}
  \end{ruledtabular}
\end{table*}
...
ADCD assigns a verdict of \unresolved, providing transparent diagnostic feedback that anomalous physics exists but cannot be formally certified without broader domain data.
For Screened Coulomb, the recovered Debye length $\theta_0 \approx 1.50$
matches the ground truth $\lambda_D = 1.50$ and achieves decisive ablation margin ($\Delta\mathrm{BIC} = 25.74$), though positive control sensitivity at $1\%$ noise yields a \unresolved tier in blind mode.
For Entropy Expansion, the logarithmic form passes all four gates with
$\Delta\mathrm{BIC} = 37.99$, achieving full \identifiable certification.
...
\setlength{\tabcolsep}{2pt}
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc}
Regime & \textsc{Identifiable} & \shortstack{\texttt{DETECTED}\\\texttt{UNRESOLVED}} & \textsc{Withheld} \\
\hline
...
intended epistemic protocol: when the data do not provide
very strong evidence over competing alternatives ($\Delta\mathrm{BIC} < 10$),
ADCD assigns a qualified verdict of \unresolved rather than reporting an unsupported
discovery.
