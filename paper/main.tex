...
\begin{enumerate}[noitemsep,leftmargin=*]
  ...
  \item \textbf{Gate D (Fine parameter fit):} Surviving candidates undergo full continuous parameter optimization using \texttt{Optim.jl} Nelder--Mead ($n_\mathrm{restarts} = 15$) in \texttt{float64} precision across five initialization scales ($10^{-6}$ to $10^6$) with quadrant sign permutations and log-uniform exponent sampling to robustly escape non-convex local minima.
