...
(e.g., trigonometric oscillations of charge) without expanding the search space
to include physically unjustified forms.
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    \textit{Right}: $\Delta\mathrm{BIC}$ (ablated minus correct; positive values
    favour the correctly-structured candidate).
...
\begin{table}[ht]
  ...
  $\Delta$BIC = Ablation Control gap (best-without-correct-primitive minus correct;
  positive values indicate the correct structure is preferred).
...
the relativistic correction is only $4.8\%$ of the observed value---while
the injected noise level is $1\%$.
