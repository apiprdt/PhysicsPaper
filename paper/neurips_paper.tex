...
it searches exclusively for the anomalous residual that must vanish at the classical limit.
...
(e.g., trigonometric oscillations of charge) without brute-force enumeration.
...
setting.
\emph{Catastrophic cancellation}: as $\Delta \to 0$, floating-point subtraction
of large nearly-equal terms destroys the gradient signal~\cite{ieee754},
...
not present in our dictionary) in a Yukawa scenario would make it act as a
...
\texttt{isothermal\_thermodynamics} & \texttt{D\_exp}, \texttt{D\_log} &
  Entropy of mixing and van der Waals equation involve $\ln$ and $e^{-\beta\epsilon}$. \\
\bottomrule
\end{tabular}
\end{table}
...
optionally composed to depth~3 with a secondary correction term.
...
is not dimensionless unless $\theta$ carries the same dimension.
...
Surviving candidates are optimized via L-BFGS-B implemented in JAX
(\texttt{float64}).
...
the anomalous correction was just beginning to emerge from classical noise.
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    but positive control fails under the historical $v\le0.3c$ window.
  }
  \label{fig:recovery}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    \textit{Left}: BIC of the correctly-structured candidate (blue) and the
    best alternative with the ground-truth primitive family excluded (gray).
    Lower BIC is preferred.
    \textit{Right}: $\Delta\mathrm{BIC}$ (correct minus ablated).
    Red dashed: Kass--Raftery ``very strong evidence'' threshold
    ($\Delta\mathrm{BIC}=10$)~\cite{kass1995}.
    Both identifiable scenarios exceed this threshold decisively.
  }
  \label{fig:bic}
\end{figure*}
...
\begin{table}[ht]
  ...
  $\Delta$BIC = Ablation Control gap (correct minus best-without-correct-primitive).
  Match level: \textit{exact} = symbolic equivalence verified; \textit{class} = correct functional family, amplitude absorbs unverifiable constants.}
  \label{tab:results}
  \centering
...
matching the ground-truth Debye length $\lambda_D=1.50$.
...
SR outputs, which return a numerical amplitude without flagging its unverifiable
physical interpretation.
$\Delta\mathrm{BIC}=14.58>10$ supports IDENTIFIABLE.

\paragraph{Time Dilation: WITHHELD as designed.}
At $v\le0.3c$, the relativistic correction factor $\gamma-1\approx4.8\%$
of the classical baseline, while the injected noise level is $1\%$.
...
The Time Dilation WITHHELD result is frequently misread as a failure.
...
IDENTIFIABLE. The threshold crossing is a falsifiable, reproducible event.

\subsection{Taxonomy as Bayesian prior, not hint}

The distinction between ``hand-feeding the answer'' and
``specifying a domain-justified prior'' is operational.
The taxonomy restricts \emph{which functional family} is admitted;
it supplies nothing about \emph{which variable} enters the argument or
\emph{what the numerical scale} is.
A physicist confronting a plasma shielding anomaly does not try sinusoidal
oscillations of electric charge: the Debye-H\"{u}ckel derivation rules them out.
ADCD makes this reasoning explicit, auditable, and reproducible.

Including a physically unjustified primitive in the taxonomy degrades
performance: such a primitive acts as a noise sponge, absorbing variance
from the physically relevant variable (here, distance $r$), collapsing
the Ablation Control BIC gap, and destroying identifiability without
improving physical interpretability.
The Phenomenon-Specific Taxonomy is not a shortcut; it is the removal of
hypotheses that the founding physics literature has already falsified.
...
The meaningful comparison is not search space size or flexibility,
but epistemic certainty: ADCD explicitly reports when data are insufficient
to support a structural claim. Table~\ref{tab:comparison} summarizes
the distinct operational regimes.
...
real observational data (e.g., SPARC galactic rotation curves for MOND-like
corrections) is a primary direction for future work.
