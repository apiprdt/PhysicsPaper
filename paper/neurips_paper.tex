...
\begin{figure*}[t]
  ...
  \caption{%
    ...
    \textit{Left (a)}: absolute BIC of the correctly-structured candidate
...
ADCD intentionally sacrifices the unbounded expressivity of general-purpose SR engines like PySR in exchange for safety, identifiability, and physical validity. While PySR can fit arbitrary data geometries by combining unconstrained mathematical operators, ADCD refuses to explore functional forms that are dimensionally inconsistent or unsupported by the Phenomenon-Specific Taxonomy. This restricts ADCD from discovering entirely novel physical phenomena that do not share structural roots with known theories, reflecting its design as a tool for the \emph{correction} of known laws rather than their \emph{ab initio} formulation.
