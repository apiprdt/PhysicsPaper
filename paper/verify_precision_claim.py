...
if __name__ == "__main__":
    ...
    # Verification 1: float32 collapses to 0.0 at u = 1e-8
    ...
    assert naive_f32 == 0.0, "Verification 1 failed: float32 did not collapse"
    
    # Verification 2: float64 ~11% relative error at u = 2e-15
    u_f64_2e15 = 2e-15
    ...
    print(f"   Naive Rel Error  : {rel_err_2e15 * 100:.2f}% (~11% verified)")
    assert 0.10 <= rel_err_2e15 <= 0.12, "Verification 2 failed"
    
    # Verification 3: float64 at u = 1e-16 (naive yields 344% error, regularized preserves 5.0e-17)
    u_f64_16 = 1e-16
    ...
    assert abs(reg_f64_16 - 5.0e-17) < 1e-18, "Verification 3 failed: reg form wrong"
    
    # Verification 4: float64 total collapse to 0.0 at u <= 1e-17
    u_f64_17 = 1e-17
    ...
    assert naive_f64_17 == 0.0, "Verification 4 failed: did not collapse"
    assert abs(reg_f64_17 - 5.0e-18) < 1e-19, "Verification 4 failed: reg form wrong"
    
    print("\nALL PRECISION VERIFICATIONS COMPLETED SUCCESSFULLY.")
