...
class InjectionRecoveryEvaluator:
        ...
        for cand_dict in candidates:
            ...
            for t1 in theta_1_grid:
                if "exp" in expr_str:
                    ...
                    basis = sc_erf(t1 * r)
                elif "jn" in expr_str or "besselj" in expr_str:
                    from scipy.special import j0 as sc_j0
                    basis = sc_j0(t1 * r)
    ...
    def run_negative_control(
        ...
        for cand_dict in candidates:
            expr_str = cand_dict["expr"]
            # Evaluate actual per-candidate fit against pure noise
            theta_1_grid = np.linspace(0.1, 2.0, 10)
            for t1 in theta_1_grid:
                if "exp" in expr_str:
                    basis = np.exp(-t1 * r)
                elif "erf" in expr_str:
